  ...
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col gap-8 px-4 py-10 sm:max-w-3xl">
      ...
      <div className="flex gap-4">
        ...
        <Link
          href="#grading"
          className="flex-1 rounded-lg border border-neutral-200 p-4 hover:border-neutral-400"
        >
          <span className="text-2xl font-semibold">{gradingQueueCount}</span>
          <p className="text-xs text-neutral-500">Awaiting grading</p>
        </Link>
      </div>
      ...
      <section className="flex flex-col gap-3">
        ...
        {courses.length === 0 ? (
          <p className="text-sm text-neutral-500">No courses yet — create your first one below.</p>
        ) : (
          <div className="flex flex-col gap-2">
            {courses.map((c) => (
              ...
              >
                ...
                <span className="text-xs text-neutral-500">{c._count.enrollments} students</span>
              </Link>
            ))}
          </div>
        )}
